...
die-off, starfish-wasting-syndrome, reviewed in \citealtp{Uthicke2009}). In
addition, many species are targeted by unregulated fisheries (e.g.,
\citealtp{Purcell2014}). Despite these factors, echinoderms have received
...
shown that species can be told apart based on their color patterns (e.g.,
\citealtp{Kim2013,Kerr2013}). However, taxonomic confusion through the years has
hindered our knowledge of species distributions, as incorrect identifications in
many species complexes are pervasive in the literature. Having photographic
evidence associated with geospatial data will allow accurate delineation of the
geographical distributions of once confused species, after taxonomic research
has clarified species limits. iNaturalist can also help track changes in species
abundance (e.g., crown-of-thorns outbreaks) and condition (e.g.,
...
\begin{center}
<<all-observations, dependson='data-prep', eval=TRUE, cache=TRUE, echo=FALSE, fig.keep='last', warning=FALSE, fig.width=8, out.width='\\columnwidth'>>=
...
{\small This article is open-source (CC-BY), fully reproducible and available on
  ...
  \texttt{ggplot2} \citep{Wickham2009}, \texttt{knitr} \citep{Xie2014},
  \texttt{taxizesoap} \citep{Chamberlain2013,Chamberlain2014}, and
  \texttt{wesanderson} \citep{Ram2014}.  }
